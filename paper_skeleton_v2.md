...
- $\theta_\# P$: pushforward of $P$ under $x \mapsto \langle \theta, x \rangle$.
...
where $\rho_{PQ} = \text{Corr}(F_P^{-1}(U), F_Q^{-1}(U))$, $U \sim \text{Unif}(0,1)$.
...
$$SW_p^p(P,Q) = \int_{\mathbb{S}^{d-1}} W_p^p(\theta_\# P, \theta_\# Q) \, d\sigma(\theta).$$
...
where $\mathcal{B}^2(\Sigma_1, \Sigma_2) = \text{tr}(\Sigma_1) + \text{tr}(\Sigma_2) - 2\text{tr}\!\left((\Sigma_1^{1/2}\Sigma_2\Sigma_1^{1/2})^{1/2}\right)$ is the squared Bures distance.
...
**KS.** $\text{KS}(P,Q) = \sup_{x} |F_P(x) - F_Q(x)|$. Univariate only.

**MMD.** For RKHS kernel $k$:
$$\text{MMD}^2(P,Q) = \mathbb{E}[k(X,X')] + \mathbb{E}[k(Y,Y')] - 2\mathbb{E}[k(X,Y)].$$

**Energy distance.** $E(P,Q) = 2\mathbb{E}\|X-Y\| - \mathbb{E}\|X-X'\| - \mathbb{E}\|Y-Y'\|$. Equals MMD$^2$ with the distance kernel (Sejdinovic et al. 2013).
...
**Lemma 2 (MMD$^2$ / Energy Mixture Bound).** *$\text{MMD}^2(P, R_\alpha) = \alpha^2 \text{MMD}^2(P,Q)$, so $h(\alpha) = \alpha^2$.*

*Proof.* Mean embedding is linear: $\mu_{R_\alpha} = (1-\alpha)\mu_P + \alpha \mu_Q$. Then:
$$\text{MMD}^2(P, R_\alpha) = \|\mu_P - \mu_{R_\alpha}\|_{\mathcal{H}}^2 = \alpha^2 \|\mu_P - \mu_Q\|_{\mathcal{H}}^2 = \alpha^2 \text{MMD}^2(P,Q). \quad \square$$

**Lemma 3 (KS Mixture Bound).** *$\text{KS}(P, R_\alpha) = \alpha \, \text{KS}(P, Q)$, so $h(\alpha) = \alpha$.*
...
| $\text{MMD}^2$ | $\alpha^2$ | $n^{-1/2}$ | $x/2$ | Lem 2; Gretton+ '12 |
| $E$ (energy) | $\alpha^2$ | $n^{-1/2}$ | $x/2$ | Lem 2 (distance kernel); Székely-Rizzo '04 |
| KS | $\alpha$ | $n^{-1/2}$ | $x$ | Lem 3; DKW |

**Observation.** All have $\phi(n) \asymp n^{-1/2}$. The contamination functions differ: $h(\alpha) = \alpha$ for Wasserstein/KS, $h(\alpha) = \alpha^2$ for MMD$^2$/energy. As we show in Theorem 1, this affects localization constants but not rates.
...
For $h(\alpha) = \alpha^2$ (MMD$^2$, energy): $\bar{h}(\rho) = 2\rho - \rho^2$, so
...
**(ii)** For $h(\alpha) = \alpha^2$ (MMD$^2$, energy):
...
**Remark (MMD$^2$ has better constant).** The factor-2 improvement for MMD$^2$ in the localization constant arises because the quadratic contamination $h(\alpha) = \alpha^2$ creates a steeper gap near $\tau^*$. However, this advantage disappears in rate: both give $r^* = \Theta(\sqrt{T \log T}/\kappa)$ with balanced segments.
...
| Cor 4 | MMD$^2$ | Kernel + full distribution | Free (but signal depends on $\sigma$, $d$) |
...
*$\text{KS}(P,Q) = \frac{c_{\text{KS}}}{\nu} + O(\nu^{-2})$ where $c_{\text{KS}} = \frac{1}{4}\max_z |z^3 - 3z|\phi(z) \approx 0.139$.*
...
Global max: $|g| = 0.554$, achieved at $x \approx 0.742$. So $c_{\text{KS}} = 0.554 / 4 = 0.139$. $\square$
...
$$\frac{r^*_{\text{KS}}}{r^*_{W_2}} = \frac{\kappa_{W_2}}{\kappa_{\text{KS}}} = \frac{W_2(P,Q)}{\text{KS}(P,Q)} = \frac{0.612 / \nu}{0.139 / \nu} \approx 4.4.$$
...
$$\text{KS}(P, Q) \to \delta, \qquad W_2^2(P, Q) \to 4\delta a^2.$$
*Consequently, $W_2 / \text{KS} \to 2a\sqrt{\delta}/\delta = 2a/\sqrt{\delta} \to \infty$.*

*Proof.* For $a \gg \sigma$, the component Gaussians become approximately point masses $\delta_{\pm a}$.

**KS.** $F_P(x) \approx 0.5 \cdot \mathbf{1}_{x \geq -a} + 0.5 \cdot \mathbf{1}_{x \geq a}$. Similarly $F_Q(x) \approx (0.5 + \delta)\mathbf{1}_{x \geq -a} + (0.5 - \delta)\mathbf{1}_{x \geq a}$. For $x \in (-a, a)$: $|F_P(x) - F_Q(x)| = |0.5 - (0.5 + \delta)| = \delta$. For $x < -a$ or $x > a$: difference is 0. So $\text{KS} \to \delta$.
...
**Localization comparison.** $r^*_W / r^*_{\text{KS}} = \text{KS} / W_2 = \delta / (2a\sqrt{\delta}) = \sqrt{\delta}/(2a) \to 0$. Wasserstein-CPD localization improves by a factor of $2a/\sqrt{\delta}$ over KS.

**Intuition.** KS sees a bounded CDF gap (the fraction of mass that moved). $W_2$ sees the CDF gap *times the distance the mass moved*. When modes are far apart, the transport cost dominates and $W_2 \gg \text{KS}$.
...
| Alternative | $W_2$ rate | KS rate | $W_2$ constant | KS constant | $W_2 / \text{KS}$ |
|---|---|---|---|---|---|
| Gaussian vs $t_\nu$ | $\nu^{-1}$ | $\nu^{-1}$ | 0.612 | 0.139 | 4.4 |
...
| MMD$^2$ (characteristic kernel) | $> 0$ (metrizes convergence) |
...
**Definition 6.** $(P, Q) \in \mathcal{S}_r(\kappa)$ if $P = \mathcal{N}(0, \Sigma)$, $Q = \mathcal{N}(0, \Sigma + E)$ where $\text{rank}(E) = r$ and $W_2^2(P,Q) \geq \kappa$.
...
**Proposition 8 (MMD$^2$ with Gaussian kernel, rank-1 shift).** *Let $P = \mathcal{N}(0, I_d)$, $Q = \mathcal{N}(0, I_d + \varepsilon vv^\top)$, kernel $k(x,y) = \exp(-\|x-y\|^2/(2\sigma^2))$. Then for $\sigma^2 = \gamma d$ (standard scaling, $\gamma > 0$ fixed):*
$$\text{MMD}^2(P, Q) = \Theta\!\left(\frac{\varepsilon^2}{d^2}\right) \quad \text{as } d \to \infty, \; \varepsilon \text{ fixed.}$$
...
Expanding more carefully to $O(d^{-2})$: each term picks up a correction $\sim \varepsilon^2 / (\gamma^2 d^2)$, and $\text{MMD}^2 = \Theta(A \varepsilon^2 / (\gamma^2 d^2)) = \Theta(\varepsilon^2 / d^2)$. $\square$
...
| $\text{MMD}^2$ (isotropic) | $\Theta(\varepsilon^2/d^2)$ | $\Theta(d^2 \sqrt{T \log T}/\varepsilon^2)$ |
...
$$W_2^2(\theta_\# P, \theta_\# Q) = (\sqrt{1 + \varepsilon(\theta^\top v)^2} - 1)^2 \approx \frac{\varepsilon^2 (\theta^\top v)^4}{4}.$$
...
| MMD$^2$ (isotropic Gaussian, $\sigma^2 = \gamma d$) | $\Theta(\varepsilon^2/d^2)$ | $d^{-2}$ dilution |
...
| Rank-1 spiked shifts | Full $W_2$ (dim-free) | SW$_2$, MMD (both $d^{-2}$) |
| Localization constant (§3) | MMD$^2$ ($h = \alpha^2$, factor 2) | $W_2$, KS |
...
3. **Full second-order expansion** for MMD$^2$ in Prop 8.
...
3. "MMD has worse localization" → **False at the theorem level.** MMD$^2$ actually has a factor-2 *better* localization constant via $h(\alpha) = \alpha^2$.
